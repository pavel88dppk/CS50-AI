...
class NimAI():
    ...
    def choose_action(self, state, epsilon=True):
        ...
        best = None
        for action in actions:
            if best is None or self.get_q_value(state, action) > best_rew:
                best_rew = self.get_q_value(state, action)
                best = action
        ...
        if epsilon:
            ...
            if random.choice(numbers) < 15:
                best = random.choices(actions)[0]
